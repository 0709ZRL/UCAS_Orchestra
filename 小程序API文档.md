...
**注册失败的两个典型情况（小程序必须把 message 原样提示）**

| HTTP | message | 触发条件 |
|---|---|---|
| **409** | `该账号已被注册` | 账号已存在 |
| **409** | `该姓名已注册账号，请勿重复创建` | 已有同名人员，且对方有登录账号 |
| **409** | `该姓名已在成员名单中，请联系管理员开通登录账号` | 已有同名人员，但对方是管理员在「成员管理」里手工添加的（无登录账号） |
| **400** | `账号、密码、姓名为必填项` | 缺字段 |

> ⚠️ **不允许同名注册**：不区分对方有没有账号，只要库里存在同名人员就拒绝。
> **没有「提前检查姓名是否可用」的接口**，只能提交后根据 409 的 message 提示用户。
...
> ⚠️ 改成功后**旧 token 仍然有效**（后端不失效 token），无需重新登录；
> 但建议小程序提示「密码已修改」并保持登录状态。

**方式二：未登录 —— 账号 + 真实姓名 校验后重置**（「忘记密码」用这个）
...
> 两个接口的密码下限都是 **6 位**，其它强度不限。
> 重置成功后请引导用户用新密码重新登录（不返回 token）。
